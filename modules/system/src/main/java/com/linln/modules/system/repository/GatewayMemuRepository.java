...
public interface GatewayMemuRepository extends BaseRepository<GatewayMemu,Long> {
     ...
     *  
     */
    List<GatewayMemu> findByPidOrderByNumber(Long pid);
     ...
     *  
     */
    @Query(value="select * from gateway_memu order by number",nativeQuery = true)
    List<GatewayMemu> findAllOrderByNumber();
    ...
     */
    @Query(value = "select * from gateway_memu where pid=0 order by number ",nativeQuery = true)
